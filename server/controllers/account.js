...
Account.prototype.login = function(email, password, callback) {
    ...
    me.user.findOne({ email: email }, function (err, user) {
        ...
        if (err) {
            return callback(err, new me.api_resp({ success: false, payload: { msg: me.api_error_messages.database_error } }));
        }
        ...
        if (user) {
            me.hashPassword(password,String(user.password_salt), function (err, passwordHash) {
                ...
                if (passwordHash == user.password_hash) {
                    ...
                    return callback(err, new me.api_resp({ success: false, payload: { msg: me.api_error_messages.invalid_password } }));
                }
            });

        } else {
            return callback(err, new me.api_resp({ success: false, payload: { msg: me.api_error_messages.not_found } }));
        }
...
Account.prototype.register = function (newUser, callback) {
    ...
    me.user.findOne({ email: newUser.email }, function (err, user) {
        ...
        if (user) {
            return callback(err, new me.api_resp({ success: false, payload: { msg: me.api_error_messages.email_already_exists } }));
        } else {
            ...
            newUser.save(function (err, user, numberAffected) {
                ...
                if (numberAffected === 1) {
                    ...
                    return callback(err, new me.api_resp({ success: false, payload: { msg: me.api_error_messages.user_create_error } }));
                }
